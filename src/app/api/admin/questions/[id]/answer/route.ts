...
import { idSchema, saveAnswerSchema } from "@/lib/validation";
...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    if (!answer) return apiError("回答不存在", 404);
...
export async function PUT(request: NextRequest, context: Context) {
  try {
    ...
    const [id, body] = [
      ...
      saveAnswerSchema.safeParse(await request.json().catch(() => null)),
    ];
    if (!id.success || !body.success) return apiError("回答内容无效");
    const result = saveAnswer(db, {
      questionId: id.data,
      content: body.data.content,
    });
    if (result.kind === "not-found") return apiError("问题不存在", 404);
